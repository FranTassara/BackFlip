...
class ChannelControl(QWidget):
    """Control widget for an individual microscopy channel."""

    def __init__(self, channel_name: str, channel_idx: int, callback, max_value: int = 255):
        super().__init__()
    ...
    def init_ui(self, channel_name: str) -> None:
        layout = QVBoxLayout()
        ...
        # Custom RGB controls (initially hidden)
    ...
    def create_slider_with_spinbox(self, label: str, min_val: int, max_val: int, default_val: int) -> dict:
        """Create a slider with label and editable SpinBox for RGB values."""
        layout = QHBoxLayout()
        lbl = QLabel(label)
    ...
    def create_slider(self, label: str, min_val: int, max_val: int, default_val: int) -> dict:
        """Create a slider with label and numeric value display."""
        layout = QHBoxLayout()
        lbl = QLabel(label)
        lbl.setMinimumWidth(90)  # Fixed width for labels

        slider = QSlider(Qt.Horizontal)
        slider.setMinimum(min_val)
        slider.setMaximum(max_val)
        slider.setValue(default_val)
        slider.setMaximumWidth(150)  # Limit slider width

        value_label = QLabel(str(default_val))
        value_label.setMinimumWidth(50)  # Fixed width for value
...
class BackFlipGUI(QMainWindow):
    """Main application window for fluorescence microscopy image composition."""

    def __init__(self):
        super().__init__()
        self.image_data: list | None = None
        self.num_channels: int = 0
        self.z_slices: int = 0
        self.projected_channels: list = []
        self.channel_controls: list = []
        self.bit_depth: int = 8
        self.max_intensity_value: int = 255

        # Attributes to track image type
        self.is_composite: bool = False  # True for RGB composite, False for multichannel
        self.composite_image: np.ndarray | None = None  # Stores the RGB composite image
        self.composite_control: CompositeControl | None = None  # Control widget for composites

        # Crop attributes
        self.crop_applied: bool = False
        self.original_image_data: list | None = None
        self.original_projected_channels: list | None = None
        self.original_composite_image: np.ndarray | None = None
    ...
    def init_ui(self) -> None:
        ...
        window_height = min(int(screen.height() * 0.9), 900)  # Maximum 900px
        window_width = min(int(screen.width() * 0.85), 1600)  # Maximum 1600px
        ...
        # Left panel - General controls (with scroll)
        ...
        # Right panel - Per-channel controls
    ...
    def create_left_panel(self) -> QWidget:
        """Create the general controls panel."""
        # Container panel
        panel = QWidget()
        ...
        scroll.setFrameShape(QScrollArea.Shape.NoFrame)  # No borders
        ...
        self.white_method_combo.addItems(["Subtractive RGB", "HSL Inversion", "YIQ Inversion", "CIELab Inversion", "Replace Gray (ezReverse)"])
    ...
    def create_center_panel(self) -> QWidget:
        """Create the center panel with preview."""
        panel = QWidget()
        layout = QVBoxLayout()
    ...
    def create_right_panel(self) -> QWidget:
        """Create the right panel with per-channel controls."""
        panel = QWidget()
        layout = QVBoxLayout()
    ...
    def load_image(self) -> None:
        """Load an image file (CZI, JPG, PNG, TIFF)."""
    ...
    def detect_tiff_type(self, file_path: str) -> str:
        """
        Detect if a TIFF file is an RGB composite or has separate fluorescence channels.
    ...
    def load_czi(self, file_path: str | None = None) -> None:
        """Load a CZI file."""
        ...
        try:
            ...
            with czi.open_czi(file_path) as czifile:
                ...
                if sample_dtype == np.uint8:
                    ...
                    # Default to assuming max value in data
                    max_val = max([np.max(ch) for ch in self.image_data])
                    if max_val <= 255:
                        self.bit_depth = 8
                        self.max_intensity_value = 255
                ...
                # Enable Z-projection selector
                self.projection_combo.setEnabled(True)

                # Create per-channel controls
                self.create_channel_controls()
    ...
    def create_composite_controls(self) -> None:
        """Create controls for RGB composite images."""
    ...
    def on_method_change(self) -> None:
        """Callback when inversion method changes."""
        method = self.white_method_combo.currentText()
        # Show/hide tolerance control based on method
        self.replace_tolerance_widget.setVisible(method == "Replace Gray (ezReverse)")
        self.update_preview()

    def on_projection_change(self) -> None:
        """Callback when projection type changes."""
    ...
    def apply_lut(self, data: np.ndarray, lut_name: str, custom_rgb: tuple) -> np.ndarray:
        """Apply a LUT to normalized data (0-255)."""
        h, w = data.shape
    ...
    def process_channel(self, channel_data: np.ndarray, settings: dict) -> np.ndarray:
        """Process a channel with all adjustments."""
        data = channel_data.copy().astype(np.float32)

        # === Optimal processing order ===

        # 1. Median filter (if enabled) - reduces salt-and-pepper noise
        if settings['bg_median']:
            kernel_size = settings['median_size']
            data = ndimage.median_filter(data, size=kernel_size)

        # 2. Top-hat filter (if enabled) - corrects uneven illumination
        ...
        # 3. Gaussian background subtraction (rolling ball alternative)
        if settings['bg_gaussian']:
            blurred = ndimage.gaussian_filter(data, sigma=settings['gaussian_sigma'])
            data = data - blurred
            data = np.clip(data, 0, None)

        # 4. Threshold (remove low intensities)
    ...
    def process_composite_image(self, img: np.ndarray, settings: dict) -> np.ndarray:
    ...
    def update_preview(self) -> None:
        """Update the composite image preview."""
    ...
    def update_preview_composite(self):
        ...
        if self.white_radio.isChecked():
            ...
            if method == "Subtractive RGB":
                composite = self.apply_subtractive_to_composite(processed)
    ...
    def apply_subtractive_to_composite(self, img):
        """Apply Subtractive RGB algorithm to RGB composite (R=255-G-B, G=255-R-B, B=255-R-G)"""
    ...
    def update_preview_multichannel(self) -> None:
        """Update the composite image preview (multichannel)."""
        if not self.projected_channels:
            return

        h, w = self.projected_channels[0].shape

        # Process each enabled channel and save in RGB format
        channel_rgb_list = []
        enabled_indices = []
        ...
        if self.white_radio.isChecked():
            ...
            if method == "Subtractive RGB":
                composite = self.compose_white_background_subtractive(channel_rgb_list, enabled_indices)
            elif method == "HSL Inversion":
            ...
            # BLACK BACKGROUND: standard additive mode
    ...
    def compose_white_background_subtractive(self, channel_rgb_list: list, enabled_indices: list) -> np.ndarray:
        """
        Compose RGB image with white background using Subtractive RGB algorithm.

        R_new = 255 - G_original - B_original
        G_new = 255 - R_original - B_original
        B_new = 255 - R_original - G_original

        This preserves fluorescence colors while converting black background to white.
        """
        h, w = self.projected_channels[0].shape
    ...
    def hls_to_rgb_custom(self, hls_image: np.ndarray) -> np.ndarray:
        """Convert HLS to RGB using colorsys (as in ezReverse)."""
    ...
    def compose_white_background_hsl(self, channel_rgb_list: list, enabled_indices: list) -> np.ndarray:
        """Compose with white background using HSL inversion."""
        h, w = self.projected_channels[0].shape

        # First create black composite
        composite = np.zeros((h, w, 3), dtype=np.uint8)
        for channel_rgb in channel_rgb_list:
            if channel_rgb is not None:
                composite = np.clip(composite.astype(np.int32) + channel_rgb.astype(np.int32), 0, 255).astype(np.uint8)

        # Use OpenCV for fast conversion
    ...
    def compose_white_background_yiq(self, channel_rgb_list: list, enabled_indices: list) -> np.ndarray:
        """Compose with white background using YIQ inversion."""
        h, w = self.projected_channels[0].shape
    ...
    def compose_white_background_lab(self, channel_rgb_list: list, enabled_indices: list) -> np.ndarray:
        """Compose with white background using CIELab inversion."""
        h, w = self.projected_channels[0].shape
    ...
    def compose_white_background_replace(self, channel_rgb_list: list, enabled_indices: list) -> np.ndarray:
        """Compose with white background replacing only grays (ezReverse method)."""
        h, w = self.projected_channels[0].shape
        ...
        # Calculate standard deviation at each pixel (across RGB channels)
        std_dev = np.std(composite_rgb.astype(np.float32), axis=2)
        
        # Create mask: True where std <= threshold (gray pixels)
        gray_mask = std_dev <= threshold

        # Create result image
        result = composite_rgb.copy()

        # Invert identified gray pixels
        result[gray_mask] = 255 - result[gray_mask]
    ...
    def _enable_crop_after_load(self) -> None:
    ...
    def toggle_crop_mode(self, checked: bool) -> None:
        """Enable/disable crop selection mode."""
    ...
    def _apply_crop_composite(self, adjusted_x: int, adjusted_y: int, w: int, h: int,
                                pixmap_width: int, pixmap_height: int) -> None:
    ...
    def _apply_crop_multichannel(self, adjusted_x: int, adjusted_y: int, w: int, h: int,
                                  pixmap_width: int, pixmap_height: int) -> None:
    ...
    def add_scale_bar(self, image: np.ndarray) -> np.ndarray:
        ...
        # Validation: bar must be visible
        if length_px < 1:
        ...
        if length_px > w * 0.9:
            ...
            length_px = int(w * 0.3)  # Reduce to 30% of width
        ...
        if show_label:
            ...
            # Text position (centered above bar)
    ...
    def export_image(self) -> None:
        """Export the composite image in the chosen format."""
        ...
        # Dialog to choose format and location
        ...
        if file_path:
            try:
                ...
                if extension in ['tiff', 'tif']:
                    # TIFF without compression (best quality)
